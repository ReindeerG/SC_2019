...
@Repository("todoDao")
public class TodoDao_Oracle implements TodoDao {
	@Autowired
	...
	public void vseqUp() {
		sqlSession.update("db_todo.vseq_1up");
	}
	
	public void insert(TodoDto todoDto) {
		this.vseqUp();
		sqlSession.insert("db_todo.insert", todoDto);
	}
	
	public void update(TodoDto todoDto) {
		sqlSession.update("db_todo.update", todoDto);
	}
	
	public void delete(int no) {
		sqlSession.delete("db_todo.delete", no);
	}
	
	public void order(TodoDto todoDto) {
		sqlSession.update("db_todo.order", todoDto);
	}
	...
	public List<TodoDto> getAllList() {
		List<TodoDto> list = sqlSession.selectList("db_todo.getalllist");
	...
	public List<TodoDto> getDeadList() {
		List<TodoDto> list = sqlSession.selectList("db_todo.getdeadlist");
		for(TodoDto todoDto : list) {
			// 마감 기한이 있는 항목이라면 날짜형식 문자열을 사용하기 위해 설정해줌.
			if(todoDto.getDeadable()==1) todoDto.setStrdeadline(this.setStrDeadline(todoDto));
		}
